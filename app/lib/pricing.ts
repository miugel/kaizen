...
function reservationSpansHoliday(start: Date, end: Date): boolean {
...
// Add new rules here to extend pricing logic.
const DISCOUNT_RULES: DiscountRule[] = [
  {
    type: "holiday",
    label: "17% holiday discount",
    applies: (start, end) => reservationSpansHoliday(start, end),
    compute: (baseTotalCents, _baseHourlyRateCents, durationInHours) => {
      const totalPriceCents = Math.round(baseTotalCents * 0.83);
      return {
        totalPriceCents,
  ...
  },
  {
    ...
    applies: (_start, _end, durationInHours) => durationInHours > 72,
    compute: (_baseTotalCents, baseHourlyRateCents, durationInHours) => {
      const effectiveHourlyRateCents = baseHourlyRateCents - 1000;
      return {
